...
class OrderService {
    ...
    fun getAll(): List<Order> = orders.values.toList()

    fun getById(id: Long): Order? = orders[id]

    fun update(id: Long, request: CreateOrderRequest): Order? {
        return if (orders.containsKey(id)) {
            val order = toOrder(id, request, orders[id]!!.status)
            orders[id] = order
            order
        } else {
            null
    ...
    fun delete(id: Long): Boolean = orders.remove(id) != null
